...
public class Main
{
		...
		for(int currHeight = 1; currHeight <= maxHeight; currHeight++)
		{
			...
			if(valid)
			{
				for(int i = 0; i < n; i++)
				{
					...
					else if(flipCount < n/2 && next.width > next.height && next.width <= currHeight)
					{
						//We can have this person also lie down, since it will decrease the width.
						//Since we have sorted in order of increasing (height - width), we know that
	...
	static class Person implements Comparable<Person>
	{
		...
		{
			return ((Integer) (height - width)).compareTo((Integer) (other.height - other.width));
		}
	}
}
